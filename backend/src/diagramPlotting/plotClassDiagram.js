    ...
    for (const file of projectJSON) {
      try {
        ...
        for (const classNode of root.descendantsOfType("class_declaration")) {
          const className =
            classNode.childForFieldName("name")?.text || "UnknownClass";
          ...
          if (bodyNode) {
            for (const member of bodyNode.namedChildren) {
              if (member.type === "field_declaration") {
                ...
                const varDecls = member.descendantsOfType(
                  "variable_declarator"
                );
                ...
                for (const varDecl of varDecls) {
                  ...
                  if (attrName && attrType) {
                    ...
                    const innerTypes =
                      this.extractClassTypeFromCollection(attrType);
              ...
              if (member.type === "method_declaration") {
                const methodName =
                  member.childForFieldName("name")?.text || "UnknownMethod";
                const methodParams =
                  member.descendantsOfType("formal_parameter");
                let returnType =
                  member.childForFieldName("type")?.text || "void";
                ...
                methods.push(
                  `${methodName}(${methodParams
                    .map((param) => {
                      const paramName =
                        param.childForFieldName("name")?.text || "arg";
                      const paramType =
                        param.childForFieldName("type")?.text || "UnknownType";
                ...
                methodParams.forEach((param) => {
                  ...
                  if (paramType) {
                    const innerTypes =
                      this.extractClassTypeFromCollection(paramType);
                    innerTypes.forEach((inner) => dependencies.add(inner));
                    dependencies.add(paramType);
                  }
                });

                const innerReturnTypes =
                  this.extractClassTypeFromCollection(returnType);
                ...
                if (methodBody) {
                  const objectCreations = methodBody.descendantsOfType(
                  ...
                  const identifiers =
                    methodBody.descendantsOfType("identifier");
            ...
            if (implementsNode) {
              const interfaceTypes =
                implementsNode.descendantsOfType("type_identifier");
              ...
              if (
                ...
                implementsNode.text.trim()
              ) {
                const directInterfaces = implementsNode.text
                  .trim()
                  .split(/\s+/);
            ...
            classes.push({
              ...
              extendsClass,
              implementsInterfaces,
              associations: Array.from(associations).filter(
                (assoc) =>
                  this.userDefinedClasses.has(assoc) || this.enums.has(assoc)
              ),
              dependencies: Array.from(dependencies).filter(
                (dep) => this.userDefinedClasses.has(dep) || this.enums.has(dep)
              ),
              file: file.filePath,
              isEnum: false,
            });
        ...
        for (const interfaceNode of root.descendantsOfType(
          "interface_declaration"
        )) {
          const interfaceName =
            interfaceNode.childForFieldName("name")?.text || "UnknownInterface";
  ...
  async generateDiagram(parsedClasses, projectPath) {
    ...
    for (const cls of parsedClasses) {
      ...
      if (cls.isEnum) {
        ...
        cls.attributes.forEach((val) => lines.push(`  ${val}`));
        lines.push("}");
      ...
      if (cls.isInterface) {
        ...
        cls.methods.forEach((method) => lines.push(`  +${method}`));
        lines.push("}");
        classBlocks.push(lines.join("\n"));
        continue;
      }
      ...
      // NORMAL CLASS
      const hasAttributes = cls.attributes.length > 0;
      const hasMethods = cls.methods.length > 0;
      const hasContent = hasAttributes || hasMethods;

      if (hasContent) {
        lines.push(`class ${cls.className} {`);
        cls.attributes.forEach((attr) => lines.push(`  +${attr}`));
        cls.methods.forEach((method) => lines.push(`  +${method}`));
        lines.push("}");
      } else {
        // empty normal class without {}
        lines.push(`class ${cls.className}`);
      }
    ...
    for (const cls of parsedClasses) {
      if (cls.extendsClass && this.userDefinedClasses.has(cls.extendsClass)) {
        const extendedClass =
          cls.extendsClass.split(" ")[1] || cls.extendsClass;
